...
SVGLineElement.prototype.getTotalLength = function() {
	return pythagore([this.getAttribute('x1'), this.getAttribute('y1')], [
		this.getAttribute(
			'x2'), this.getAttribute('y2')
	])
}
...
SVGPolylineElement.prototype.getTotalLength = function() {
	...
	for (var i = 0, l = points.length - 2; i < l; i++) {
		distance += pythagore(points[i].split(',')
			.map(parseFloat), points[i + 1].split(',')
			.map(parseFloat))
	}
	return distance
}
...
CSSStyleSheet.prototype.setRule = function(selector, rule) {
	...
	function toCssAttribute(attribute) {
		return attribute.replace(/[A-Z]/, '-$&', 'g')
			.toLowerCase()
	}
}
...
function getUnitInPixels(unit) {
	...
	var height = div.getBoundingClientRect()
		.height
...
ANIMATE = localStorage.getItem('animate') === null ? document.getElementById(
		'animation_check')
	.checked : localStorage.getItem('animate')
...
TAGS_CHECK = document.getElementById('tags')
...
LOGO_ARROW_PATH.style.strokeDasharray = logo_body_length + ' ' +
	logo_body_length * 3
...
WebFontConfig = {
	google: {
		families: ['Droid+Serif::latin', 'Permanent+Marker::latin']
	},
			...
			if (FIRST_LANDING)
				...
				for (var i = 0, l = GRAPHS.length; i < l; i++) {
					if (GRAPHS[i].is_processed) {
						server_and_console.log('graph #' + i +
							' being reprocessed after font load')
						var old_writing = GRAPHS[i].content.querySelectorAll(
							'[data-type=writing]')
...
document.getElementById('animation_check')
	.checked = ANIMATE
document.getElementById('speed_input')
	.value = SPEED

// EXTRACT SVG DATA FROM DOM (it starts in the DOM for SEO purposes)
if (ARCHIVES) {
	for (var i = 0; i < AS.length; i++) {
		GRAPHS[i].content = AS[i].firstElementChild.removeChild(AS[i].firstElementChild
			.firstElementChild)
	}
} else {
...
function add_all_event_listeners() {
	for (var i = 0; i < AS.length; i++) {
		AS[i].addEventListener('click', navigate.bind(undefined, i))
	}
	document.getElementById('home')
		.addEventListener('click', navigate.bind(undefined, 0))
	...
	document.getElementById('archives')
		.addEventListener('click', function(event) {
			if (ARCHIVES) {
				event.stopPropagation();
				event.preventDefault();
			}
			navigate('archives', event)
		})
	document.getElementById('speed_input')
	...
	for (var i = 0; i < TAGS_CHECK.length; i++) {
		TAGS_CHECK[i].addEventListener('change', filter)
	}
...
if (ARCHIVES)
	...
	if (recovered_index !== -1) {
		server_and_console.log('state recovered from history: asking for graph #' +
			recovered_index + ': @ ' + GRAPHS[recovered_index].name)
		setup_graph(recovered_index)
	} else {
		server_and_console.log('creating history state: asking for graph #' + INDEX +
			': @ ' + GRAPHS[INDEX].name)
		window.history.replaceState({
			name: GRAPHS[INDEX].name
		}, "", GRAPHS[INDEX].name)
		setup_graph(INDEX)
	}
}
...
function setup_graph(index) {
	...
	if (!ARCHIVES && index === currently_loading_index) {
		server_and_console.warn('will not setup graph: asking for #' + index + ' (' +
			GRAPHS[index].formatted_name + ') and already loading/showing #' +
			currently_loading_index + ' (' + GRAPHS[currently_loading_index].formatted_name +
			') (INDEX is #' + INDEX + ': ' + GRAPHS[INDEX].formatted_name + ')');
		return
	}
	...
	if (ARCHIVES) {
		...
		load_svg(index, function(index) {
			...
			server_and_console.log('graph #' + index +
				' loaded (coming back from ARCHIVES)');
			then(index)
		})
	} else {
	...
	function then(index) {
		if (currently_loading_index !== index)
			return
		server_and_console.log('cloning #' + index + ' processed ' + (GRAPHS[index]
			.nofont ?
			'without' : 'with') + ' font')
		MAIN.innerHTML = ''
		...
		// release date
		var date_obj = new Date(GRAPHS[index].release[1] + ' / ' + GRAPHS[index]
			.release[
				2] + ' / ' + GRAPHS[index].release[0])
		var pubdate = document.createElement('span')
		pubdate.id = 'pubdate'
		pubdate.textContent = 'published on ' + date_obj.getLitteralMonth() + ' ' +
			parseInt(GRAPHS[index].release[2]) + date_obj.getDatePostfix() + ', ' +
			GRAPHS[index].release[0]
		...
		if (GRAPHS[index].gif_is_loaded)
			...
			if (DOMURL.createObjectURL) {
				...
				server_and_console.warn(
					'there isnt going to be any img nor gif overlay for this one: #' + index +
					' (' + GRAPHS[index].formatted_name + ')')
		}
	...
	if (expanded_as.length > 0) {
		...
		for (var i = 0, l = expanded_as.length; i < l; i++) {
			...
			setTimeout((function(el) {
					if (el.className === 'unexpanded') el.className = ''
				})
				.bind(undefined, expanded_as[i]), 1000)
		}
	}
...
function which_vignette_should_load() {
	...
	which_vignette_should_load_id = window.requestAnimationFrame(
		check_each_vignette_for_loading_position)
}
...
function check_each_vignette_for_loading_position(force) {
	...
	for (var i = 0, l = AS.length; i < l; i++) {
		if (GRAPHS[i].is_processed || GRAPHS[i].being_loaded)
			delete loading_as[i]
		else if (loading_as[i] && !loading_as[i].hidden && !loading_as[i].requested &&
			a_is_in_window(i, loading_window)) {
			...
			if (force === true) {
				...
				setTimeout((function(index, margin) {
						...
						if (loading_as[index] && !loading_as[index].hidden &&
							a_is_in_window(
								index, loading_window)) {
...
function a_is_in_window(i, loading_window) {
	return (AS[i].offsetTop > loading_window[0] && AS[i].offsetTop <
		loading_window[1])
}
...
function filter(event) {
	...
	for (var i = 0; i < tag_list.length; i++) {
		var counter = document.getElementById('tags')
			.querySelector('label[for=' + TAGS_CHECK[i + 1].getAttribute('id') +
				'] div')
...
function sizes_have_changed(aside_changed, section_changed) {
	...
	server_and_console.log('height is now based on ' + (base_height_on_aside ?
		'aside' : 'section'));
...
function position_main_slide(ref_rect) {
	SHEET.setRule('section a.expanded > div', {
		transform: 'translate(calc(65vw - ' + (ref_rect.left + (ref_rect.width +
				REM) / 2) + 'px), calc(50vh - ' + (ref_rect.top + ref_rect.height / 2) +
			'px)) scale(' + ((.7 * cached_inner_width - REM) / ref_rect.width) +
			', ' +
			((cached_inner_height - 2 * REM) / ref_rect.height) + ')'
	})
}

function update_logo_colors(index) {
	var color = GRAPHS[index].content.querySelector(
			'svg path[stroke], svg polyline[stroke]')
		.getAttribute('stroke')
...
function load_svg(index, callback, increment) {
	...
	if (GRAPHS[index].being_loaded && !GRAPHS[index].is_processed) {
		server_and_console.warn('graph #' + index + ' (' + GRAPHS[index].formatted_name +
			') was already requested, queing up callbacks')
	...
	httpRequest.onreadystatechange = loaded_svg.bind(undefined, index,
		httpRequest,
		increment)
	...
	function loaded_svg(index, httpRequest, increment) {
		if (httpRequest.readyState === XMLHttpRequest.DONE) {
			if (httpRequest.status === 200) {
				...
				if (increment < 10)
					setTimeout(load_svg.bind(undefined, index, undefined, increment + 1),
						1000)
				else {
					server_and_console.warn('couldnt load graph ' + index + ' (' + GRAPHS[
						index].formatted_name + ') after ' + increment + ' trials, giving up.')
	...
	function preprocess_svg(index) {
		if (!GRAPHS[index].is_processed) {
			...
			var erase = GRAPHS[index].content.removeChild(GRAPHS[index].content.querySelector(
				'[stroke="#FFFFFF"]'))
...
function load_watermark_from_server(img, index) {
	server_and_console.log('loading watermarked img for #' + index +
		' from server')
...
function load_gif_from_server(img, index) {
	...
	temp_gif.onload = (function(img, temp_gif, index) {
			server_and_console.log('GIF loaded for #' + index + ' from server @ ' +
				GRAPHS[index].gif)
	...
	temp_gif.onerror = (function(img, temp_gif, index) {
			server_and_console.warn('error loading GIF for #' + index + ' @ ' +
				temp_gif.src)
		})
		.bind(undefined, img, temp_gif, index)
...
function save_gif_img_to_server(img, index, img_nb, img_total, callback) {
	if (!GRAPHS[index].nofont) {
		...
		var request = new XMLHttpRequest()
		request.open('POST', '/save_gif_img.php?name=' + GRAPHS[index].name +
			'&img_nb=' + img_nb + '&img_total=' + img_total, true);
		request.setRequestHeader('Content-type', 'application/x-www-form-urlencoded')
		request.onreadystatechange = (function(img, index, request, img_nb,
				img_total, callback) {
				if (request.readyState === 4 && request.status === 200) {
					if (img_nb == 0) server_and_console.log('GIF processed for #' + index)
					else server_and_console.log('gif img for #' + index +
						' sucessfuly uploaded (' + img_total + ' total images)')
					if (request.responseText)
						GRAPHS[index].gif = request.responseText
					callback(index, img_nb, img_total, true)
				} else if (request.readyState === 4 && request.status !== 200) {
					server_and_console.warn('ERROR: GIF img ' + img_nb + ' of ' + img_total +
						' for #' + index + ' NOT uploaded. Server responded with ' + request.statusText +
		...
		if (img_nb == 0)
			server_and_console.log('uploading last img for #' + index +
				' and processing GIF')
		request.send(params)
	} else {
		server_and_console.warn('NOT uploading gif img for #' + index +
			' to server because of font kerning issues')
	}
}

function save_img_to_server(urldata, index) {
	server_and_console.log('uploading img for #' + index + ' to server, ' + (
		GRAPHS[index].nofont ? 'wont' : 'will') + ' save image')
	var params = 'dataURL=' + encodeURIComponent(urldata)
	var request = new XMLHttpRequest();
	request.open('POST', '/save_img.php?name=' + GRAPHS[index].name +
		'&nosave=' +
		(GRAPHS[index].nofont ? 'true' : 'false'), true);
	request.setRequestHeader('Content-type', 'application/x-www-form-urlencoded')
	request.onreadystatechange = (function(index, request) {
			if (request.readyState === 4 && request.status === 200) {
				server_and_console.log('img for #' + index + ' sucessfuly uploaded @ ' +
					request.responseText)
...
function create_gif(index, callback_create_gif) {
	...
	var total_duration = get_svg_anim_duration(GRAPHS[index].content, false) *
		1000
	var nb_of_imgs = Math.floor(total_duration / 50) + 1
	server_and_console.log('creating GIF for ' + GRAPHS[index].formatted_name +
		', gif duration: ' + Math.round(total_duration / 1000) + 's')

	svg_to_png(index, 100, (function(data, png_data_url, percent, callback) {
			img = new Image()
			img.src = png_data_url
			save_gif_img_to_server(img, data.index, data.current_img, data.nb_of_imgs, (
					function(data, callback, index, img_nb, img_total, success, error) {
						if (success)
							do { // TODO: does upload work reliably?
								data.current_img--
						...
						if (data.current_img >= 0) {
							...
							setTimeout(svg_to_png.bind(undefined, data.index, data.current_img /
								data.nb_of_imgs, callback), 0)
						} else
							data.callback()
					})
...
function try_to_rewrite_with_paths(graph, font_loaded, callback) {
	if (graph.nofont !== false) {
		try {
			...
			try {
				server_and_console.warn('couldnt rewrite #' + graph.id +
					' with font, removing previous attempt (' + graph.formatted_name + ')')
				var messed_up_writings = graph.content.querySelectorAll(
					'[data-type=writing]')
				...
				server_and_console.warn('couldnt rewrite #' + graph.id +
					' at all. this is a big problem. (' + graph.formatted_name + ')')
				var messed_up_writings = graph.content.querySelectorAll(
					'[data-type=writing]')
				for (var i = 0, l = messed_up_writings.length; i < l; i++) {
					messed_up_writings[i].parentNode.removeChild(messed_up_writings[i])
	...
	server_and_console.log('rewriting #' + graph.id + ' ' + (font_loaded ?
			'with' :
			'without') + ' font loaded, ' + (graph.nofont ? 'didnt' : 'did') +
		' use font')
	if (callback)
		callback()
}
...
function rewrite_with_paths(svg, font_loaded) {
	...
	function replace_span(reference_element, font_loaded, is_text_long) {
		if (reference_element.childNodes.length > 1 || reference_element.childNodes[
				0]
			.nodeType !== 3) {
		...
		var text_transform = is_tspan ? reference_element.parentElement.getAttribute(
			'transform') : reference_element.getAttribute('transform')

		var color = reference_element.getAttribute('fill')
		if (!color)
			color = is_tspan ? reference_element.parentElement.getAttribute('fill') :
			false

		var sentence = reference_element.childNodes[0].nodeValue
		var insert_at = is_tspan ? reference_element.parentElement :
			reference_element
		for (var char_pointer = 0; char_pointer < sentence.length; char_pointer++) {
			...
			if (!letter) {
				server_and_console.warn('There is no letter \'' + sentence.charAt(
					char_pointer) + '\' in the alphabet.')
				continue
			}
			if (font_loaded)
			...
			for (var i = 0; i < paths.length; i++) {
				if (!font_loaded) {
					...
					paths[i].setAttribute('transform', text_transform + ' translate(' + x +
						',' + y + ')')
				} else {
					paths[i].setAttribute('transform', text_transform + ' translate(' +
						letter_pos.x + ',' + (letter_pos.y - letter.viewbox.height + 10) + ')'
					)
				}
	...
	function compute_letter(letter) {
		...
		var viewbox = (el.getAttribute('viewbox') || el.getAttribute('viewBox'))
			.split(' ')
		letter.viewbox = {
...
function get_svg_anim_duration(svg, start) {
	...
	iterate_group(svg, function(element) {
		if (['g', 'line', 'polyline', 'path'].indexOf(element.tagName.toLowerCase()) ===
			-1)
			return
...
function interrupt_drawing_element_svg(svg) {
	iterate_group(svg, function(element) {
		...
		element.style.strokeDasharray = computedStyle.getPropertyValue(
			'stroke-dasharray')
		element.style.strokeDashoffset = computedStyle.getPropertyValue(
			'stroke-dashoffset')
		...
		if (element.getAttribute('data-type') === 'erase') {
			...
			if (element_starts_at < current_time) {
				...
				if (element_starts_at + element_duration < current_time) {
					...
					element.style.strokeDashoffset = element_length - (current_time -
						element_starts_at) / element_duration * element_length
				}
			}
...
function start_drawing_element(element, delay, start, callback) {
	...
	if (start) {
		...
		element.style.transition = 'stroke-dashoffset ' + duration + 's ' +
			get_element_smoothing_type(element) + ' ' + delay + 's, opacity 0s ' +
			delay +
			's'
		element.style.strokeDashoffset = '0'
...
function get_element_anim_duration(element) {
	...
	if (element.getAttribute('data-type') === 'writing' || !element.getAttribute(
			'stroke')) {
...
function logo_move_along(duration, origin, timestamp) {
	...
	LOGO_ARROW_HEAD.style.transform = 'translate(' + (point.x - 149.8) + 'px, ' +
		(point.y - 6) + 'px)'
	LOGO_ARROW_PATH.style.strokeDashoffset = logo_body_length - (1 + ratio) *
		logo_full_path_length
	if (delta < duration)
		window.requestAnimationFrame(logo_move_along.bind(undefined, duration,
			origin))
	else
